...
\note{Zazwyczaj używane tylko w dialekcie pustynnym. Patrz \emph{on}. Wykorzystywane w związkach frazeologicznych.}
...
\dictterm{n}{(\textsc{pl} ageji [ˈa.gɛ.ʐi]) (\textsc{fem}) sałata}
...
\note{Używane w~kontekście, że podmiot tak skupiał się na czymś, że umknął mu pewien ważny fakt.}
...
\dictterm{n}{(\textsc{pl} bolukaji [ˈbɔ.lu.ka.ʐi]) (\textsc{fem}) oddział (wojskowy), pułk, obecnie odpowiednik pułku, tj. około 1000 żołnierzy}
...
\note{Używany w~toastach, np. \emph{churche ti} oznacza \emph{za ciebie}, a~\emph{churche Eygeride} to \emph{za Cesarstwo}.}
...
\dictterm{pro}{pan (on)}
\note{Zaimki epié oraz epiá są bardzo formalne. Używa się ich tylko w~języku formalnym, w odniesieniu do obcych osób albo osób stojących wyżej w~hierarchii, albo w~sytuacji, kiedy nie znamy imienia osoby, do której chcemy się zwrócić. Mogą być używane zarówno jako 3SG, jak i~2SG. W~niektórych dialektach stosowane również jako przyrostki, w szczególności do funkcji np. \emph{falazer-epié} -- ,,pan dowódca''.}
...
\dictterm{n}{(\textsc{pl} falazeyaji [ˈfa.la.zɛ.ja.ʐi]) (\textsc{fem}) pluton (wojskowy), obecnie odpowiednik plutonu, tj. około 25 ludzi}
...
\note{Dosłownie: liczby-maszyna. Jest to jedno z najdłuższych słów w języku andro i stąd najczęściej stosuje się skróconą formę, \emph{hu͞ekapa}.}
...
\dictterm{v}{(\textsc{pst} iént [i.ˈɛŋt]) przychodzić, przybyć, przybywać; \emph{iéni a͞u} -- pochodzić (z)}
...
\dictterm{n}{(\textsc{pl} isdarji [ˈis.dar.ʐi]) (\textsc{fem}) odpowiednik stopnia dywizji, zgrupowanie wojsk lądowych liczące obecnie około 10 tysięcy żołnierzy}
...
\dictterm{v}{(\textsc{pst} jarant [ˈʐa.raŋt]) rozwiązywać (problem, zagadkę), ale także: rozwiązywać sznury, więzy}
...
\dictterm{n}{(\textsc{pl} jenosaji [ˈʐɛ.nɔ.sa.ʐi]) (\textsc{fem}) drużyna, w~szczególności drużyna jako oddział wojskowy, grupa około 5 żołnierzy}
...
\note{Dla każdego liczebnika można wykorzystać określenie \emph{razi}, na przykład \emph{ka razi} oznacza dwukrotnie, dwa razy.}
...
\note{Forma używana dawniej. Obecnie raczej \emph{kapata}, w~niektórych słowach ewoluowało wręcz do członu \emph{-kapa}, patrz \emph{hu͞ekapa}.}
...
\note{W sytuacji, w~której nie chcemy określać płci, używa się rodzaju męskiego. Słowo to dotyczy głównie miłości romantycznej, bo kochanek w~sensie seksualnym to \emph{aḿachager}.}
...
\dictword{lanomaŕida}[la.nɔ.ma.ˈri.da]
\dictterm{n}{(\textsc{pl} lanomaŕidi [la.nɔ.ma.ˈri.di]) (\textsc{fem}) biblioteka}
...
\note{\emph{lipe kay}, \emph{lipe tay}, \emph{lipe zevoyim} to typowe pozdrowienia i przywitania -- dobrej nocy, dobrego dnia, dobrego poranka.}
...
\note{W~tradycyjnej formie zapisu nazwiska zawsze poprzedza się nazwę rodu partykułą \emph{mal}, na przykład \emph{Koolder mal Erlehirni}.}
...
\note{Określenie to pochodzi od faktu, że przed wiekami sprawy sądowe były rozpatrywane wewnątrz rodzin, które to potem przedstawiały jaki wyrok został podjęty.}
...
\note{Słowo \emph{maltorn} pochodzi od słów \emph{mal} (rodzina) oraz \emph{torn}, który jest wrostkiem często oznaczającym coś związanego z~murami, twierdzami -- aczkolwiek samo \emph{torn} pochodzi od \emph{tori} (rzucać), ale jest często stosowane w nazwach miast.}
...
\note{Używany rzadko, zwykle w dialekcie południowo-wschodnim. Raczej używane jest \emph{je}, \emph{niger} dotyczy rzeczy, które nie są w zasięgu wzroku.}
...
\dictterm{part}{określa wielokrotność, używane w~połączeniu z~liczebnikiem, na przykład \emph{ka razi} -- dwukrotnie}
...
\note{W rodzaju żeńskim słowo to uzyskuje końcówkę <ea>, jednak nie wymawia się tego jako [ˈri.gɛa], a jako [ˈri.gɛ.a].}
...
\note{Istnieje wyrażenie \emph{Seysei choke chu ti}, oznaczające dosłownie ,,dzięki tobie poczułem się miło'', używane jako odpowiednik ,,miło mi było ciebie poznać''.}
...
\dictterm{v}{(\textsc{pst} uýut [u.ˈjut]) ogłuchnąć, stracić słuch, ale również: \emph{ge uýut} -- zostać uciszonym}
...
\note{Dawniej używane było również jako partykuła w imionach: \emph{Keyer val Dorigan} - Keyer, syn Dorigana. Ciekawostką jest to, że forma ta wyglądała identycznie dla kobiet - \emph{Keyla val Dorigan}. Używana było tylko jeśli rodzic (najczęściej ojciec) był powszechnie znaną osobistością.}
...
\note{Bardziej popularna forma, ale w~dialekcie pustynnym pojawia się określenie \emph{vyage}.}
...
\note{Istnieje szereg idiomów używanych wraz z~operatorem \emph{vige}: \emph{vige choke tay} -- miłego dnia, \emph{vige menede} -- na zdrowie, \emph{vige sekupa vayara} -- bezpiecznej podróży (dosłownie tłumaczy się na: ,,niech twoja podróż będzie bezpieczna'').}
...
\dictterm{adj}{spragniony (domyślnie: napoju, wody), ale również może być spragniony czegokolwiek innego -- używa się tutaj partykuły \emph{chu}, np. \emph{voese chu koóln} -- spragniony miłości}
...
\note{Używane jako pejoratywne określenie osoby, w~szczególności osoby, która sypia z~wieloma innymi osobami. W~starożytności niewolnicy musieli używać w stosunku do siebie określenia \emph{je zaámey} lub wręcz \emph{che zaámey} zamiast zaimka \emph{mi}.}
...
\note{Używane również jako pożegnanie, w~mowie potocznej skracane często do \emph{zay} -- ,,pa'', ,,pa-pa''.}
